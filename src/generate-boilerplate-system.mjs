...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'daggerheart', 'Daggerheart', and 'DAGGERHEART'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.daggerheart/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.daggerheart/g),
   ...
   * Rename files.
   *
   * Rename files that had daggerheart in their name, such as
   * css/daggerheart.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*daggerheart*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('daggerheart', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-daggerheart-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Daggerheart's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace daggerheart name mentions in files.
      generator.replaceFileContents();
      // Rename files that had daggerheart in their name.
